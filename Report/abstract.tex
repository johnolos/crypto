\begin{abstract}
A good pseudorandom number generator (PRNG) is an essential ingredient in creating a secure cryptographic system. The output numbers of those number generators are used futher in encrypting and decrypting messages. Since the process of generating these numbers is deterministic a security system will have a major security breach if those numbers produced are predictable either by having a predictable output pattern or simply do not have a "random" enough seed input for it's number generation. The goal is to have an easy to produce pseudorandom number generator than generates numbers as close to random as possible. There is essentially a tradeoff between having a complicated good PRNG and having a PRNG that is fast, but also secure enough to use in production.
